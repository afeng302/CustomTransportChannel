...
namespace WebSocketChannel
{
    public class HttpConnectProxy : ProxyConnectorBase
    {
        ...
        private const string m_RequestWithAuthTemplate = "CONNECT {0}:{1} HTTP/1.1\r\nHost: {0}:{1}\r\nProxy-Connection: Keep-Alive\r\nProxy-Authorization: Basic {2}\r\n\r\n";
        ...
        protected override void ProcessConnect(Socket socket, object targetEndPoint, SocketAsyncEventArgs e)
        {
            ...
            if (targetEndPoint is DnsEndPoint)
            {
                ...
                if (this.NeedAuthentication)
                {
                    request = string.Format(m_RequestWithAuthTemplate, targetDnsEndPoint.Host, targetDnsEndPoint.Port, authInfo);
                }
                else
                {
            ...
            else
            {
                ...
                if (this.NeedAuthentication)
                {
                    request = string.Format(m_RequestWithAuthTemplate, targetIPEndPoint.Address, targetIPEndPoint.Port, authInfo);
                }
                else
                {
        ...
        private bool NeedAuthentication
        {
            get
            {
